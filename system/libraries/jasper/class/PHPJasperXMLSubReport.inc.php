...
class PHPJasperXMLSubReport{
	...
	private $xoffset=0;
	public $printSubRelato;
    public function PHPJasperXMLSubReport($lang="pt-BR",$pdflib="TCPDF",$xoffset=0){
        $this->lang=$lang;
        error_reporting(1);
        $this->pdflib=$pdflib;
    ...
    public function xml_dismantle($xml, $arrayNoSubSelect, $select) {
        ...
        foreach ($xml as $k=>$out) {
            switch($k) {
                ...
                case "queryString":
//                 	print_r($select);
                    $this->queryString_handler($select);
                    break;
                case "field":
...
//read level 0,Jasperreport page setting
    ...
    public function parameter_handler($xml_path, $arrayNoSubSelect) {
       ...
       $this->printSubRelato = true;
       if ($arrayNoSubSelect != null) {
       		foreach ($arrayNoSubSelect as $value) {
       			if ($value == $xml_path["name"]) {
...
//        $b=$data->subreportParameter;
    ...
    public function transferDBtoArray($host,$user,$password,$db_or_dsn_name,$cndriver="mysql") {
        ...
        if($cndriver=="odbc") {
            ...
            $this->dataEmpty = mysql_num_rows($result) == 0;
//             echo $this->dataEmpty.'<br/>';
    ...
    public function detail() {
        ...
        if ($this->arraysqltable) {
        	foreach ($this->arraysqltable as $row) {
                ...
                foreach($this->arraydetail as $compare)	{
                    ...
                    switch($compare["hidden_type"]) {
                        case "field":
                            ...
                            if (isset($this->arraygroup[$this->group_name]["groupFooter"]) &&
                                (($checkpoint+($compare["height"]*$txt))>($this->arrayPageSetting["subreportpageHeight"]-$this->arraygroupfootheight-$this->arrayPageSetting["bottomMargin"]-$this->BottomHeightFromMainPage))) {      
                            	$this->pageFooter($checkpoint);
                                $checkpoint=$this->arraydetail[0]["y_axis"];
                                $biggestY=0;
                                $tempY=$this->arraydetail[0]["y_axis"];
                            } else if (isset($this->arraypageFooter) &&
                            		   (($checkpoint+($compare["height"]*($this->NbLines($compare["width"],$txt))))>($this->arrayPageSetting["subreportpageHeight"]-$this->arraypageFooter[0]["height"]-$this->arrayPageSetting["bottomMargin"]-$this->BottomHeightFromMainPage))) { 
                            	$this->pageFooter($checkpoint);
                                $headerY = $this->arrayPageSetting["topMargin"]+$this->arraypageHeader[0]["height"];
                                $this->pageHeaderNewPage();
                            ...
                            } else if (isset($this->arraylastPageFooter) &&
                            		   (($checkpoint+($compare["height"]*($this->NbLines($compare["width"],$txt))))>($this->arrayPageSetting["subreportpageHeight"]-$this->arraylastPageFooter[0]["height"]-$this->arrayPageSetting["bottomMargin"]-$this->BottomHeightFromMainPage))) { 
                ...
                if ($checkpoint+$this->arraydetail[0]["height"]>($this->arrayPageSetting["pageHeight"]-$this->arraypageFooter[0]["height"]-$this->arrayPageSetting["bottomMargin"] - $ghheight -$ghfoot)) {
                    $this->pageFooter($checkpoint); 
                    $this->background();
                    $headerY = $this->arrayPageSetting["topMargin"]+$this->arraypageHeader[0]["height"];
                    $this->pageHeader($headerY);
...
//                     $biggestY=0;
                ...
                if ($biggestY>$checkpoint+$this->arraydetail[0]["height"]) {
                    $checkpoint=$biggestY;
                } else if($biggestY<$checkpoint+$this->arraydetail[0]["height"]) {
