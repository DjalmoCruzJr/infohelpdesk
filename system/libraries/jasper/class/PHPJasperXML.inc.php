...
class PHPJasperXML {
    ...
    private $detailPrinter=1;
    private $countSubReport=0;
    private $arrayNoSubSelect;
        ...
        foreach ($xml as $k=>$out) {
            ...
            switch($k) {
                ...
                case "subDataset":
                    ...
                    break;
                case "background":
                	// Alterado por Luiz, Pablo, Esdras, Washington no dia 19/08/2015
...
//read level 0,Jasperreport page setting
    ...
    /**
     * Function alterado por Luiz Mário 27/08/2015
     * Mudanças do totalizador de registro
     * */
    public function variable_handler($xml_path) {
        ...
        $this->arrayVariable["$xml_path[name]"]=array("calculation"=>$xml_path["calculation"]."",
        	...
        	"incrementType"=>$xml_path["incrementType"]."",	
        	"incrementGroup"=>$xml_path["incrementGroup"]."",
...
//          					   $this->arrayVariable[$xml_path["name"]."_COUNT"] = $this->arrayVariable[$xml_path["name"]."_COUNT"] + 1;
...
//        $b=$data->subreportParameter;
                ...
                $srrepalcearr=array('.jrxml',"","",'','assets/relatorios/');
...
//                     $out["target"]= $this->analyse_expression( $out['target'], true);
    ...
    public function outpage($out_method="I", $arrayNoSubSelect, $filename="") {
    		$this->arrayNoSubSelect = $arrayNoSubSelect;
...
//     echo     print_r( $this->group_count,true)."<br/>";
    ...
    if($arraydata["type"]=="SetFont") {
        ...
        elseif($arraydata["type"]=="subreport") {	
        	...
        	$resultado = $this->runSubReport($arraydata,$y_axis);
        	
//         	if ($resultado == 748 || $this->pdf->GetY() == 748) {
//         		echo '760<br>';
// 				return 760;
//         	} else {        	
	            if ($resultado != 0) {
...
//                $arrdata[$num]=str_replace('"',"",$out);
    ...
    public function includeSubReport($d,$arrdata,$current_y){ 
               ...
               if (!$PHPJasperXMLSubReport->dataEmpty && $PHPJasperXMLSubReport->printSubRelato) {  
